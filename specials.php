...
<?php
	...
	if (($col === 0) && ($row != 0)) {
	  $specials_prods_content .= '<ul class="row_separator"><li class="prods_hseparator">'.tep_draw_separator('spacer.gif', '1', '1').'</li></ul>';
	} 
	if ($col === 0) {
...
// *************************************
// *************************************
  	$p_pic = '<a class="prods_pic_bg" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '" style="width:'.(SPECIAL_IMAGE_WIDTH + PIC_MARG_W).'px;height:'.(SPECIAL_IMAGE_HEIGHT + PIC_MARG_H).'px;">' . tep_image(DIR_WS_IMAGES . $specials['products_image'], $specials['products_name'], (SPECIAL_IMAGE_WIDTH), (SPECIAL_IMAGE_HEIGHT), ' style="width:'.(SPECIAL_IMAGE_WIDTH + PIC_MARG_W2).'px;height:'.(SPECIAL_IMAGE_HEIGHT + PIC_MARG_H2).'px;margin:'.PIC_MARG_T.'px '.PIC_MARG_R.'px '.PIC_MARG_B.'px '.PIC_MARG_L.'px;"') . '';
  	$p_name = '<span><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '">' . $specials['products_name'] . '</a></span>';
		
		if	(MAX_DESCR_SPECIALS !=0)	{
			$p_desc =  '<div class="desc desc_padd">'.mb_substr(strip_tags($product['products_description']), 0, MAX_DESCR_SPECIALS, 'UTF-8').'...</div>';
		}else{
			$p_desc =  '';	
		}
		
  	$p_price = '<span class="productSpecialPrice" style="float:right;">' . $currencies->display_price($specials['specials_new_products_price'], tep_get_tax_rate($specials['products_tax_class_id'])) . '</span> <del>' . $currencies->display_price($specials['products_price'], tep_get_tax_rate($specials['products_tax_class_id'])) . '</del>';
// *************************************  
...
// *************************************
    ...
    $specials_prods_content .= '<li style="width:' . PRODS_BLOCK_SPECIALS_WIDTH . 'px;" class="wrapper_prods equal-height3 hover">'. "\n".
				 
	
				  '<div class="border_prods">'. "\n".
								
					'	<div class="pic_padd wrapper_pic_div" style="width:'.(SPECIAL_IMAGE_WIDTH + PIC_MARG_W).'px;height:'.(SPECIAL_IMAGE_HEIGHT + PIC_MARG_H).'px;">'.$p_pic.'</a></div>'. "\n". //'.tep_draw_specials_pic_top().''.tep_draw_specials_pic_bottom().'
          ''. "\n".	
