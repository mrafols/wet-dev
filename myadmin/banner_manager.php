  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'setflag':
        ...
        break;
      case 'setflag_target':
        if ( ($HTTP_GET_VARS['flag_target'] == '0') || ($HTTP_GET_VARS['flag_target'] == '1') ) {
          tep_set_banner_target($HTTP_GET_VARS['bID'], $HTTP_GET_VARS['flag_target']);
      ...
      case 'update':
		...
		$banners_target = tep_db_prepare_input($HTTP_POST_VARS['banners_target']);
...
<?php
  if ($action == 'new') {
    ...
    if (isset($HTTP_GET_VARS['bID'])) {
      ...
      $banner_query = tep_db_query("select banners_title, banners_url, banners_image, banners_group, banners_html_text, banners_target, status, date_format(date_scheduled, '%Y/%m/%d') as date_scheduled, date_format(expires_date, '%Y/%m/%d') as expires_date, expires_impressions, date_status_change from " . TABLE_BANNERS . " where banners_id = '" . (int)$bID . "'");
      $banner = tep_db_fetch_array($banner_query);
      ...
      <tr><?php echo tep_draw_form('new_banner', FILENAME_BANNER_MANAGER, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo tep_draw_hidden_field('banners_id', $bID); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="main"><?php echo tep_draw_textarea_field('banners_title', 'soft', '60', '5', $bInfo->banners_title); ?></td>
          </tr>
          <tr>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_TARGET; ?></td>
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_STATUS; ?></td>
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_ACTION; ?>&nbsp;</td>
              </tr>
<?php
    $banners_query_raw = "select banners_id, banners_title, banners_image, banners_target, banners_group, status, expires_date, expires_impressions, date_status_change, date_scheduled, date_added from " . TABLE_BANNERS . " order by banners_group, banners_title";
...
<?php
      if ($banners['banners_target'] == '0') {
		echo '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $banners['banners_id'] . '&action=setflag_target&flag_target=1') . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', 'Open in Same windows', 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', 'Open in New windows', 10, 10);      } else {
		echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', 'Open in Same windows', 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $banners['banners_id'] . '&action=setflag_target&flag_target=0') . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', 'Open in New windows', 10, 10) . '</a>';
      }
?>                
                </td>                
                <td class="dataTableContent" align="right">
