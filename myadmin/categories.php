...
// removing categories can be a lengthy process
          ...
          while (list($key) = each($products_delete)) {
			...
			// Tag Cloud V1.0 (fix by Sesman 05.04/2012)  smarty delete the tags associated with this product... and  tags name associated with this product...          
					$tag_query = tep_db_query("select tag_id from products_tags where products_id = '" . $key . "'");
					while ($tag = tep_db_fetch_array($tag_query)) {
						$tag_id = $tag['tag_id'];
						$other_prods = tep_db_query("select products_id from products_tags where tag_id = '" . $tag_id . "' and products_id <> '" .$key. "'");
						if (tep_db_num_rows($other_prods)<1) {
							tep_db_query("delete from tags where tag_id = '" . $tag_id . "'");
						}
					}
					tep_db_query("delete from products_tags where products_id = '" . $key . "'");
...
// Tag Cloud V1.0 (fix by Sesman 05.04/2012)  smarty delete the tags associated with this product... and  tags name associated with this product...          
		$tag_query = tep_db_query("select tag_id from products_tags where products_id = '" . (int)$product_id . "'");
		while ($tag = tep_db_fetch_array($tag_query)) {
			$tag_id = $tag['tag_id'];
			$other_prods = tep_db_query("select products_id from products_tags where tag_id = '" . $tag_id . "' and products_id <> '" .(int)$product_id. "'");
			if (tep_db_num_rows($other_prods)<1) {
				tep_db_query("delete from tags where tag_id = '" . $tag_id . "'");
			}
		}
