...
// Sets the target of a banner
  function tep_set_banner_target($banners_id, $banners_target) {
    if ($banners_target == '1') {
      return tep_db_query("update " . TABLE_BANNERS . " set banners_target = '1', date_banners_target_change = now(), date_scheduled = NULL where banners_id = '" . (int)$banners_id . "'");
    } elseif ($banners_target == '0') {
      return tep_db_query("update " . TABLE_BANNERS . " set banners_target = '0', date_banners_target_change = now() where banners_id = '" . (int)$banners_id . "'");
    } else {
      return -1;
    }
  }
...
// Display a banner from the specified group or banner id ($identifier)
  function tep_display_banner($action, $identifier) {
    if ($action == 'dynamic') {
      ...
      if ($banners['count'] > 0) {
        $banner = tep_random_select("select banners_id, banners_title, banners_image, banners_target, banners_html_text from " . TABLE_BANNERS . " where status = '1' and banners_group = '" . $identifier . "'");
      } else {
      ...
      if (is_array($identifier)) {
        ...
        $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_target, banners_html_text from " . TABLE_BANNERS . " where status = '1' and banners_id = '" . (int)$identifier . "'");
    ...
    if (tep_not_null($banner['banners_html_text'])) {
      $banner_string = $banner['banners_html_text'];
		...
		if ($banner['banners_target']){
			$banner_target = '_self';	
		}else{
			$banner_target = '_blank';	
		}
      $banner_string = '<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner['banners_id']) . '" target="'.$banner_target.'">'. tep_image(DIR_WS_IMAGES . $banner['banners_image'], $banner['banners_title'], '', '') . '</a>';
    }
...
// Check to see if a banner exists
  function tep_banner_exists($action, $identifier) {
    if ($action == 'dynamic') {
      return tep_random_select("select banners_id, banners_title, banners_image, banners_target, banners_html_text from " . TABLE_BANNERS . " where status = '1' and banners_group = '" . $identifier . "'");
    } elseif ($action == 'static') {
      $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_target, banners_html_text from " . TABLE_BANNERS . " where status = '1' and banners_id = '" . (int)$identifier . "'");
