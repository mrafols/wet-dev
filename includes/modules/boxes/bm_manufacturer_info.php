  ...
  class bm_manufacturer_info {
    ...
    function bm_manufacturer_info() {
      ...
      if ( defined('MODULE_BOXES_MANUFACTURER_INFO_STATUS') ) {
		...
		if	(MODULE_BOXES_MANUFACTURER_INFO_CONTENT_PLACEMENT == 'Left Column')	{
			$this->group = 'boxes_column_left';
		}
		if	(MODULE_BOXES_MANUFACTURER_INFO_CONTENT_PLACEMENT == 'Right Column')	{
			$this->group = 'boxes_column_right';
		}
		if	(MODULE_BOXES_MANUFACTURER_INFO_CONTENT_PLACEMENT == 'Product Page')	{
	  ...
	  if (isset($HTTP_GET_VARS['products_id'])) {
        ...
        if (tep_db_num_rows($manufacturer_query)) {
		  ...
		  $data = '<div class="infoBoxWrapper" style="width:'.$width_ext.'px;">' . tep_draw_box_wrapper_top() .
                  '  <div class="infoBoxHeading prod_page">' . tep_draw_box_title_top() . MODULE_BOXES_MANUFACTURER_INFO_BOX_TITLE . tep_draw_box_title_bottom() . '</div>' .
                  '  <div class="infoBoxContents">' . tep_draw_box_content_top() . $manufacturer_info_string . tep_draw_box_content_bottom() . '</div>'. "\n".
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Content Placement', 'MODULE_BOXES_MANUFACTURER_INFO_CONTENT_PLACEMENT', 'Left Column', 'Should the module be loaded in the left or right column?', '6', '1', 'tep_cfg_select_option(array(\'Left Column\', \'Right Column\', \'Product Page\'), ', now())");
