...
*/
  ...
  class cm_banner_set {
    ...
    function cm_banner_set() {
      ...
      if ( defined('MODULE_BOXES_BANNER_SET_STATUS') ) {
        ...
        $this->pages = MODULE_BOXES_BANNER_SET_DISPLAY_PAGES;
				$this->group = ((MODULE_BOXES_BANNER_SET_CONTENT_PLACEMENT == 'Under Header Block') ? 'boxes_under_header' : 'boxes_above_footer');
    ...
    function execute() {
	  ...
	  if (($banner1 = tep_banner_exists('dynamic', BANNER_SET_GROUP_1)) || ($banner2 = tep_banner_exists('dynamic', BANNER_SET_GROUP_2)) || ($banner3 = tep_banner_exists('dynamic', BANNER_SET_GROUP_3)) ){
		  
	  $data1 = '<ul class="banner_set">';
	  }
	  $execute = false;
	  
	  if ( ($banner1 = tep_banner_exists('dynamic', BANNER_SET_GROUP_1))){
        $data1 .= '<li class="item1">'.tep_display_banner('static', $banner1).'</li>';
		$execute = true;		
	  }
	  if ( ($banner2 = tep_banner_exists('dynamic', BANNER_SET_GROUP_2))){
        $data1 .= '<li class="item2">'.tep_display_banner('static', $banner2).'</li>';
		$execute = true;		
	  }
	  if ( ($banner3 = tep_banner_exists('dynamic', BANNER_SET_GROUP_3))){
        $data1 .= '<li class="item3">'.tep_display_banner('static', $banner3).'</li>';
		$execute = true;		
	  }
	  if (($banner1 = tep_banner_exists('dynamic', BANNER_SET_GROUP_1)) || ($banner2 = tep_banner_exists('dynamic', BANNER_SET_GROUP_2)) || ($banner3 = tep_banner_exists('dynamic', BANNER_SET_GROUP_3)) ){
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Content Placement', 'MODULE_BOXES_BANNER_SET_CONTENT_PLACEMENT', 'Under Header Block', 'The module should be loaded in the under header or above footer block', '6', '1', 'tep_cfg_select_option(array(\'Under Header Block\', \'Above Footer Block\'), ', now())");
