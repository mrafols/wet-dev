  ...
  class cm_search {
    ...
    function execute() {
      ...
      $data = '<div class="search">'."\n".
			  ...
			  '	  		</div>' ."\n".
			  '	   	</div>' ."\n".
			  '		'.tep_image_submit('button_header_search.gif', 'alt="Search"', ' class="button_header_search fl_left"').'' ."\n".
			//  '		<div class="advanced"><a href="' . tep_href_link(FILENAME_ADVANCED_SEARCH) . '">'.tep_draw_button_header_search_top().'<span class="ui-button-text">'.MODULE_BOXES_ITEM_ADVANCED_SEARCH.'</span>'.tep_draw_button_header_search_bottom().'</a></div>' ."\n".              
				'</form>' ."\n". 			  
			  '</div>'."\n";
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Content Placement', 'MODULE_BOXES_SEARCH_HEADER_CONTENT_PLACEMENT', 'Header Block', 'Should the module be loaded in the Header Block?', '6', '1', 'tep_cfg_select_option(array(\'Header Block\'), ', now())");
