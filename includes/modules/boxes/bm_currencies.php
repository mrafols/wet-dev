...
*/
  ...
  class bm_currencies {
      ...
      if (substr(basename($PHP_SELF), 0, 8) != 'checkout') {
        if (isset($currencies) && is_object($currencies) && (count($currencies->currencies) > 1)) {
          ...
          $data = '<div class="infoBoxWrapper">' . tep_draw_box_wrapper_top() .
                  '  <div class="infoBoxHeading">' . tep_draw_box_title_top() . MODULE_BOXES_CURRENCIES_BOX_TITLE . tep_draw_box_title_bottom() . '</div>' .
                  ...
                  '    ' . tep_draw_pull_down_menu('currency', $currencies_array, $currency, 'onchange="this.form.submit();" class="select"') . $hidden_get_variables . tep_hide_session_id() . '</form>' .
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_BOXES_CURRENCIES_SORT_ORDER', '2060', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
