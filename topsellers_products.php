...
<?php
  ...
  $best_sellers_split = new splitPageResults($best_sellers_query_raw, MAX_DISPLAY_TOPSELLERS_PRODUCTS);
...
<?php echo tep_draw_result1_top(); ?> 
        ...
        <div class="cl_both result_top_padd ofh">
        	<div class="fl_left result_left"><?php echo $best_sellers_split->display_count(TEXT_DISPLAY_NUMBER_OF_PRODUCTS_NEW); ?></div>
            <div class="fl_right result_right"><?php echo TEXT_RESULT_PAGE . ' ' . $best_sellers_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></div>
        </div>
...
<?php
    $best_sellers_query = tep_db_query($best_sellers_split->sql_query);
	  ...
	  $best_sellers_content = '<div class="prods_table">';
	
    while ($best_sellers = tep_db_fetch_array($best_sellers_query)) {

		if (($col === 0) && ($row != 0)) {
		  $best_sellers_content .= '<div class="prods_hseparator">'.tep_draw_separator('spacer.gif', '1', '1').'</div>'; 
		} 
		if ($col === 0) {
		  $best_sellers_content .= '';
	   }else {
		   $best_sellers_content .= ''; // <td class="prods_vseparator">'.tep_draw_separator('spacer.gif', '1', '1').'
	   }	
      ...
      $product = tep_db_fetch_array($product_query);
       	...
       	$p_desc =  mb_substr(strip_tags($product['products_description']), 0, MAX_DESCR_TOPSELLERS_PRODUCTS, 'UTF-8').'...';
        $p_id = $product['products_id'];
		$p_pic = '<a class="prods_pic_bg" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $best_sellers['products_id']) . '" style="width:'.(SMALL_IMAGE_WIDTH + PIC_MARG_W).'px;height:'.(SMALL_IMAGE_HEIGHT + PIC_MARG_H).'px;">' . tep_image(DIR_WS_IMAGES . $best_sellers['products_image'], $best_sellers['products_name'], (SMALL_IMAGE_WIDTH), (SMALL_IMAGE_HEIGHT), ' style="width:'.(SMALL_IMAGE_WIDTH + PIC_MARG_W2).'px;height:'.(SMALL_IMAGE_HEIGHT + PIC_MARG_H2).'px;margin:'.PIC_MARG_T.'px '.PIC_MARG_R.'px '.PIC_MARG_B.'px '.PIC_MARG_L.'px;"') . '';
		$p_name = '<span class="name"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $best_sellers['products_id']) . '">' .$best_sellers['products_name'] . '</a></span>';
		
		$p_data = '<span>' . TEXT_DATE_ADDED . '</span> ' . tep_date_long($best_sellers['products_date_added']) . '';
		$p_manufacturers = '<span>' . TEXT_MANUFACTURER . '</span> ' . $best_sellers['manufacturers_name'] . '';

 
   	  if ($new_price = tep_get_products_special_price($best_sellers['products_id'])) {
      $products_price = '<span class="productSpecialPrice">' . $currencies->display_price($new_price, tep_get_tax_rate($best_sellers['products_tax_class_id'])) . '</span><del>' . $currencies->display_price($best_sellers['products_price'], tep_get_tax_rate($best_sellers['products_tax_class_id'])) . '</del>';
			$sale	= '<div class="sale"></div>';
// *************************************		
      } else {
// *************************************		  
      $products_price = '<span class="productSpecialPrice">'.$currencies->display_price($best_sellers['products_price'], tep_get_tax_rate($best_sellers['products_tax_class_id']).'').'</span>';
      $sale	= '';
			}
...
// style="width:'.$col_width.'"
    $best_sellers_content .= '<div class="contentInfoBlock hover">
